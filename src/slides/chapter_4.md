...
# XỬ LÝ ẢNH & THỊ GIÁC MÁY TÍNH - CHƯƠNG 4: PHÂN ĐOẠN ẢNH
...
**Định nghĩa toán học**
Chia không gian ảnh R thành n vùng con R1, R2,..., Rn thỏa mãn 5 điều kiện:

1. Hợp của tất cả Ri = R (Phân đoạn phải đầy đủ).
2. Mỗi Ri là một tập hợp liên thông.
3. Ri ∩ Rj = ∅ với mọi i ≠ j (Các vùng phải rời rạc).
4. Q(Ri) = TRUE (Các pixel trong cùng một vùng thỏa mãn một vị từ Q).
5. Q(Ri ∪ Rj) = FALSE với hai vùng lân cận (Hai vùng kề nhau phải khác biệt).
...
- Gradient của ảnh f(x, y) được biểu diễn bởi vector: ∇f(x, y) = [∂f/∂x, ∂f/∂y]
- Gradient cho biết mức thay đổi cường độ mạnh nhất tại mỗi điểm ảnh.

**Các tính chất quan trọng**

- **Biên độ của gradient (Magnitude):**
  M(x, y) = √((∂f/∂x)² + (∂f/∂y)²)
  Giá trị này càng lớn thì khả năng tại đó càng gần biên.
- **Hướng của gradient (Direction):**
  α(x, y) = arctan((∂f/∂y) / (∂f/∂x))
...
- Gx phát hiện thay đổi theo chiều ngang, Gy phát hiện thay đổi theo chiều dọc.
- Độ lớn gradient thường được tính từ Gx và Gy, từ đó xác định vị trí biên.
...
- Công thức: g(x, y) = 1 nếu M(x, y) > T, ngược lại g(x, y) = 0.
...
**Bài tập thực hành: Phát hiện biên với toán tử Sobel và phân ngưỡng**
...
- Công thức: g(x, y) = 1 nếu f(x, y) > T, ngược lại g(x, y) = 0.
...
**Bài tập thực hành: So sánh phân ngưỡng toàn cục và cục bộ**
...
2. **Tạo ảnh điều kiện $f_Q$:** $f_{Q(x, y)} = 1$ nếu $Q(x, y) = TRUE$, ngược lại bằng 0.
...
- Kết hợp khác biệt màu ($d_{lab}$) và khác biệt vị trí ($d_{xy}$):
  $$D = \sqrt{(d_{lab}/S)^2 + (d_{xy}/m)^2}$$
